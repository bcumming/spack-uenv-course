...
Specifically an OCI/DockerHubv2  container registries.
...
## Write ReFrame tests for uenv

**Providing ReFrame tests that provide appropriate cover will be a hard requirement for official UENV in 2024-q3.**

We will provide some concrete examples as soon as we can start testing on Todi again.

### Step 1.: create a reframe test in the CSCS test repository

Clone the  the `alps` branch of the `cscs-reframe-tests` repo:

https://github.com/eth-cscs/cscs-reframe-tests

Create a test: for many applications and tools we have existing tests from Daint / Eiger that can be updated.

### Step 2.: Add an `extras/reframe.yaml` file to your recipe

This file describes which "features" your uenv provides.
...
build-modules:
  ...
  activation:
    uenv modules use
    module load cuda cray-mpich hdf5 fftw openblas
gromacs:
  features:
    - cuda
    - mpi
...
## create a PR - the tests will run for you automatically

https://gitlab.com/cscs-ci/ci-testing/webhook-ci/mirrors/551234120955960/1440398897047560/-/pipelines/1313222315
...
* make it the first compiler toolchain
...
* Fortran modules generated by different toolchains are incompatible
    - the `.mod` files provided must match the Fortran compiler.


# Let's walk through updating a package

**... scratch that - Alps is not available today ...**

MCH have a request: https://jira.cscs.ch/browse/SD-61623

First, create a branch of the `alps-uenv` repository and update a recipe.

Make a PR, and launch a CI job: https://github.com/eth-cscs/alps-uenv/pull/111
